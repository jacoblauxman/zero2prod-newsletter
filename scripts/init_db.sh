...
# Setting DB connection string variables:
...
# Checks whether dockerized postgres DB already running (skips if so)
if [[ -z "${SKIP_DOCKER}" ]]
then
...
# Connects sqlx to postgres instance and runs migrations
DATABASE_URL=postgres://${DB_USER}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${DB_NAME}
export DATABASE_URL
sqlx database create
sqlx migrate run

>&2 echo "Postgres db migrated, ready to go!"
